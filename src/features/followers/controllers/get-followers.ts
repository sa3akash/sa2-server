...
export class GetFollowers {
  public async userFollowing(req: Request, res: Response): Promise<void> {
    ...
    const cachedFollowees: IFollowerData[] = await followerCache.getFollowerFromCache(`following:${req.currentUser!.userId}`);
  ...
  public async userFollowers(req: Request, res: Response): Promise<void> {
    ...
    const cachedFollowers: IFollowerData[] = await followerCache.getFollowerFromCache(`followers:${req.params.userId}`);
