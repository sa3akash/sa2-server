...
class ChatService {
  ...
  public async addMessageToDB(data: IMessageData): Promise<void> {
    const conversation: IConversationDocument = (await ConversationModel.findById(data.conversationId)) as IConversationDocument;
  ...
  /**
   *
   * get convarsation and last message
   *
   */
   ...
   *
   */
  public async getMessages(senderId: string, receiverId: string, sort: Record<string, 1 | -1>): Promise<IMessageData[]> {
    const query = {
      $or: [
  ...
  /**
   *
   * mark message as deleted to database
   *
   */
  ...
  /**
   *
   * mark message as read to database
   *
   */
  ...
  /**
   *
   * update message reaction to database
   *
   */
